# PaxGo Documentation Portal

Welcome to the official documentation for **PaxGo**.

PaxGo is a personal transformation engine that helps you turn big dreams into actionable micro-actions using AI coaching.

## 📖 Documentation Sections

### [Technical Documentation](./technical-docs.md)

Deep dive into the architecture, technology stack, and database schema.

### [Business Proposal & Strategy](./business-proposal.md)

Understanding the problem, target audience, and monetization model.

### [The PaxGo Story](./story.md)

A narrative journey of how the project was built, challenges faced, and lessons learned.

### [Privacy Policy](./privacy.md)

Official legal document for Play Store compliance.

### [Google Play Subscriptions](./google-play-subscriptions.md)

Step-by-step guide for setting up revenue and products in the Play Console.

### [Service Account Setup](./google-play-service-account.md)

How to link Google Play to RevenueCat using a JSON credentials key.

---
*Built with ❤️ for PaxGo*
