...
import { useProofs, usePermissionSlips, useActions } from '@/hooks/useStorage';
...
export default function VaultScreen() {
  ...
  const { slips: permissionSlips } = usePermissionSlips();
  ...
  const [sealingProof, setSealingProof] = useState<ProofEntry | null>(null);

  // Filter sealed proofs
  const sealedProofs = useMemo(() => {
    return proofs.filter((p) => p.isSealed);
  }, [proofs]);

  const filteredProofs = useMemo(() => {
    if (filter === 'all') return sealedProofs;
    return sealedProofs.filter((p) => p.category === filter.slice(0, -1));
  }, [sealedProofs, filter]);

  // Core insights from 5 Whys (stored as permission slips)
  const coreInsights = useMemo(() => {
    return permissionSlips.slice(0, 3); // Most recent insights
  }, [permissionSlips]);
  ...
  const handleProofPress = async (proof: ProofEntry) => {
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + spacing.md }]}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerSubtitle}>Archive of Achievement</Text>
        </View>
          ...
          <BlurView intensity={95} tint="dark" style={styles.modalOverlay}>
            ...
            >
                ...
                <View style={styles.proofImageContainer}>
                  ...
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          selectedProof.category === 'insight'
                            ? colors.champagneGold + '30'
                            : colors.vibrantTeal + '30',
                      },
                    ]}
                  />
                </View>
              ...
              <View style={styles.proofDetailContent}>
                {/* AI Vision Comment */}
                {selectedProof.aiVisionComment && (
                  <View style={styles.aiCommentBubble}>
                    <View style={styles.gabbyAvatar}>
                      <Text style={styles.gabbyAvatarText}>G</Text>
                    </View>
                    <View style={styles.aiCommentContent}>
                      <Text style={styles.aiCommentLabel}>Gabby sees:</Text>
                ...
                {/* Voice summary */}
                {selectedProof.voiceSummary && (
                  <View style={styles.voiceSummaryCard}>
                    <Text style={styles.voiceSummaryLabel}>Daily Reflection:</Text>
                    <Text style={styles.voiceSummaryText}>{selectedProof.voiceSummary}</Text>
                  </View>
                )}
